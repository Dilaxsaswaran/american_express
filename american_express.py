...
#     https://colab.research.google.com/drive/1LJRwewrmPrynNPAv7UMgQIZANRzlFO5O
...
from sklearn.metrics import roc_curve, roc_auc_score, _accuracy_score, confusion_matrix
...
def _null_details(threshold : np.int8 = 50, cols: np.ndarray = df_train.columns, df: pd.DataFrame = df_train) -> pd.DataFrame:
...
def remove_cols(df):
    return _null_details(80, df.columns).index
...
_null_details(0, categorical_cols)

# pd.set_option('display.max_rows', None)
_null_details(0, num_cols)
...
_null_details(0, df_train.columns, df_train)

df_train = de_noise(df_train)
...
nans = _null_details(0, df_test.columns, df_test)
print(nans)
print(nans.shape)
...
_null_details(0, df_test.columns, df_test)

df_test = de_noise(df_test)
...
def _amex_metric(y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
    ...
    def top_percent_captures(y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
    ...
    def nor_weighted_gini(y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        y_true_pred = y_true.rename(columns={'target': 'prediction'})
        return weighted_gini(y_true, y_pred) / weighted_gini(y_true, y_true_pred)

    g = nor_weighted_gini(y_true, y_pred)
    d = top_percent_captures(y_true, y_pred)

    return 0.5 * (g + d)

def _amex_metric_mod(y_true, y_pred):
...
_accuracy_score(y_test, y_pred.loc[:, [3]])

_amex_metric_mod(y_test.values, y_prob.iloc[:, [7]].values.flatten())

_amex_metric_mod(y_test.values, lgb_pred)
